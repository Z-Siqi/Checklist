...
@Composable
internal fun URLTypeStateCard(
    clearFocusState: MutableState<Boolean>,
    textState: TaskModify.Detail.TypeState.URL,
    onStateChange: (TaskModify.Detail.TypeState.URL) -> Unit,
    isSmallScreenSize: Boolean,
) {
    val focus = LocalFocusManager.current
    val state = rememberTextFieldState(initialText = textState.url)
    LaunchedEffect(Unit) { // Update change (when recomposed)
        if (state.text.toString() != textState.url) {
            state.clearText()
            state.edit { insert(0, textState.url) }
        }
    }
    LaunchedEffect((state.text != textState.url)) { // Callback change
        onStateChange(textState.copy(url = state.text.toString()))
    }
    ...
    TextField(
        state = state,
        placeholder = {
            if (!isSmallScreenSize) Text("${stringResource(R.string.url)}...")
        },
...
@Preview
@Composable
private fun URLTypeStateCardPreview() {
    URLTypeStateCard(
        clearFocusState = remember { mutableStateOf(false) },
        textState = TaskModify.Detail.TypeState.URL("Preview Text"),
        onStateChange = {},
